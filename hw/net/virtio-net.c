...
static void virtio_net_set_features(VirtIODevice *vdev, uint64_t features)
{
    ...
    if (virtio_has_feature(features, VIRTIO_NET_F_CTRL_GUEST_OFFLOADS)) {
        ...
        n->host_hdr_len = n->guest_hdr_len;
        n->has_rsc_hdr = 1;
        printf("Guest support dynamica offload, use new header, new host header len: %d, guest header len: %d\n", (int)n->host_hdr_len, (int)n->guest_hdr_len);
                                     ...
                                     struct iovec *iov, unsigned int iov_cnt)
{
    ...
    if (cmd == VIRTIO_NET_CTRL_GUEST_OFFLOADS_SET) {
        ...
        printf("Handle setting guest offload: supported 0x%lx, request 0x%lx\n", supported_offloads, offloads);
...
static int32_t virtio_net_flush_tx(VirtIONetQueue *q)
{
    ...
    for (;;) {
        ...
        struct virtio_net_hdr_mrg_rxbuf *pmhdr;

        pmhdr = (struct virtio_net_hdr_mrg_rxbuf *)&rsc_hdr;
        ...
        if (n->has_vnet_hdr) {
            ...
            if (n->needs_vnet_hdr_swap) {
                virtio_net_hdr_swap(vdev, (void *) &pmhdr);
                sg2[0].iov_base = &pmhdr;
